...
def logo(n):
	return str(f"""
╠───{M}Author:ArynzClem
╠───{M}Team:MUSLIM CYBER ARMY
╠───{M}Friend:Shark21,Bjorka,Lord Lex Spookybunny
╠───{M}Github:htyps://github.com/ArynzClem
...
print("\033[92m SUBSCRIBE MY CHANNEL")
...
def login():
	...
	try:
		...
		slebew = ('\nHello World!\n\nKOMENTAR DI TULIS SAMA BOTNYA RIAN-XD\n\n[ Pukul %s WIB ]\n- %s, %s -'%(jam,_hari_,hari_ini))
...
def otomatis():
	global ok,cp
	print('╠──────────────────────────────────────────────────────────────────────────────╣')
	print(f'╠───[{hh}!{P}] {hh}RIAN-XD Ok Di Simpan Di : {sim_ok}{P}\n╠───[{hh}!{P}] {K}RIAN-XD Cp Di Simpan Di : {sim_cp}{P}')
	print('╠──────────────────────────────────────────────────────────────────────────────╣')
	awal = datetime.now()
	with tred(max_workers=30) as babas:
		for akun in id:
